<template>
  <div class="home">
    ...
    <shopList :shopList="shopList" ref="inputRef"></shopList>
...
import { getLunbotu, mockGet } from "@/utils/api/index";
export default defineComponent({
  ...
  setup(props, cxt) {
    ...
    console.log(cxt, "cxt");
    axios.post("/api/post",{pagesize:1,pageIndex:10}).then((res) => {
      console.log(res, "Res");
    });
    //获取轮播图数据
    ...
    const inputRef = ref();
    const tabShowRef = () => {
      console.log(inputRef, 5555);
    };
    tabShowRef();

    return { ...toRefs(state), inputRef, tabShowRef };
  },
});
</script>
