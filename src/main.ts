...
createApp(App).use(VideoPlayer).use(Button).use(Divider).use(Card).use(Stepper).use(Checkbox).use(CheckboxGroup).use(NavBar).use(Tabbar).use(VanImage).use(ActionBar).use(ActionBarIcon).use(ActionBarButton).use(TabbarItem).use(Swipe).use(SwipeItem).use(Grid).use(GridItem).use(List).use(Cell).use(Tab).use(Tabs).use(Toast).use(Icon).use(CellGroup).use(Form).use(Field).use(SwipeCell).use(Search).use(Skeleton).use(store).use(router).mount('#app')
